...
export async function splitBinderPage(
  ...
  if (cellWidth < 1 || cellHeight < 1) {
    throw new Error(`Image (${width}x${height}) is too small for a ${grid.rows}x${grid.cols} grid`)
  }
  ...
  for (let row = 0; row < grid.rows; row++) {
    for (let col = 0; col < grid.cols; col++) {
      ...
      const extractWidth = col === grid.cols - 1 ? width - left : cellWidth
      const extractHeight = row === grid.rows - 1 ? height - top : cellHeight
