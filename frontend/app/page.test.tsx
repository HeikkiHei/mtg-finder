...
describe("Home", () => {
  it("renders the title and the binder upload section", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "MTG Finder", level: 1 }),
    ).toBeInTheDocument()
    expect(
      screen.getByRole("heading", { name: /scan a binder page/i }),
    ).toBeInTheDocument()
  })
  ...
  it("renders saved cards returned by the API", async () => {
    fetchMock.mockResolvedValue({
      ok: true,
      json: async () => [
        { id: 1, name: "Sol Ring", type: "Artifact", rarity: "uncommon" },
      ],
    })
    render(<Home />)

    expect(
      await screen.findByText(/Sol Ring - Artifact \(uncommon\)/),
    ).toBeInTheDocument()
